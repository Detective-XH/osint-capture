...
const DEFAULT_NEW_SCHEMA_COLUMNS = [
...
function _ensureDefault(stored) {
  if (!stored) {
...
function _cloneDefault() {
  return {
    ...
    is_default: true,
    columns:    DEFAULT_SCHEMA.columns.map(c => ({ ...c })),
  };
}
...
export async function getSchemaById(id) {
  ...
  return stored.schemas.find(s => s.id === id) || null;
}
...
export async function saveSchema(schema) {
  ...
  const idx = stored.schemas.findIndex(s => s.id === schema.id);
  if (idx >= 0) {
    ...
    const existingLower = new Set(
      stored.schemas.filter(s => s.id !== schema.id).map(s => s.name.toLowerCase())
    );
    ...
    if (isStub) {
      schema = {
        ...schema,
        columns: DEFAULT_NEW_SCHEMA_COLUMNS.map(c => ({
    ...
    // (EXT-UI-RESTRUCTURE Step 1c)
    const existingLower = new Set(stored.schemas.map(s => s.name.toLowerCase()));
...
export async function duplicateSchema(id) {
  const stored = _ensureDefault(await _load());
  const source = stored.schemas.find(s => s.id === id);
...
export async function setActiveSchema(id) {
  const stored = _ensureDefault(await _load());
  const exists = stored.schemas.some(s => s.id === id);
...
export async function importSchema(json) {
  const { schema_name, columns } = json || {};
  ...
  const existingLower = new Set(stored.schemas.map(s => s.name.toLowerCase()));
  const uniqueName = _uniqueName(schema_name.trim(), existingLower);
...
export async function exportSchema(id) {
  const schema = await getSchemaById(id);
  if (!schema) throw new Error(`Schema not found: ${id}`);
  return {
    schema_name: schema.name,
    columns:     schema.columns.map(c => ({ id: c.id, name: c.name, source: c.source })),
  };
}
